...
<nav
    ...
    <ul class="navbar-nav">
        ...
        <li class="nav-item d-none d-sm-inline-block">
            <a href=""
                class="nav-link"><?php echo (!isMobileDevice()) ? $_settings->info('name').' - Manager':$_settings->info('short_name').' - Mgr'; ?></a>
        </li>
    ...
    <ul class="navbar-nav ml-auto">
        ...
        <div class="">
            <li class="nav-link">
                <a href="<?php echo base_url ?>manager/?page=volunteer/request"
        ...
        <div class="">
            <li class="nav-link">
                <a href="<?php echo base_url ?>manager/?page=donation/index"
                    class="btn btn-warning btn-sm position-relative">
                    <i class="fas fa-coins"></i> Donations (rwf)

                    <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                        <?php 
                    $request = $conn->query("SELECT id FROM `donations` WHERE status = 'pending' ")->num_rows;
                    echo number_format($request);
                  ?>
                        <span class="visually-hidden">+</span>
                    </span>
            ...
            <div class="btn-group nav-link">
                <button type="button" class="btn btn-rounded badge badge-light dropdown-toggle dropdown-icon"
                    ...
                    <span><img src="<?php echo validate_image($_settings->userdata('avatar')) ?>"
                            class="img-circle elevation-2 user-img" alt="Manager Image"></span>
                    <span
                        class="ml-3"><?php echo ucwords($_settings->userdata('firstname').' '.$_settings->userdata('lastname')) ?></span>
                    <span class="sr-only">Toggle Dropdown</span>
                </button>
                <div class="dropdown-menu" role="menu">
                    <a class="dropdown-item" href="<?php echo base_url.'manager/?page=user' ?>"><span
...
<!-- /.navbar -->
<script>
// Add error handling and console logging for any issues
</script>