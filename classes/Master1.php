...
Class Master extends DBConnection {
	...
	function save_camp(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `camp_list` set {$data} ";
		}else{
			$sql = "UPDATE `camp_list` set {$data} where id = '{$id}' ";
		}
		$check = $this->conn->query("SELECT * FROM `camp_list` where `name` = '{$name}' ".(is_numeric($id) && $id > 0 ? " and id != '{$id}'" : "")." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = 'Camp Name already exists.';
			...
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id))
					$resp['msg'] = "Camp has successfully added.";
				else
					$resp['msg'] = "Camp details has been updated successfully.";
			}else{
				$resp['status'] = 'failed';
	...
	function delete_camp(){
		extract($_POST);
		$del = $this->conn->query("UPDATE `camp_list` set delete_flag = 1 where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Camp has been deleted successfully.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_refugee(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `refugees` set {$data} ";
		}else{
			$sql = "UPDATE `refugees` set {$data} where id = '{$id}' ";
		}
		$check = $this->conn->query("SELECT * FROM `refugees` where id = '{$id}' ".(!empty($id) ? " and id != '{$id}' " : "")." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "An error occured.";
		}else{
			...
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id))
					$resp['msg']= " Refugee Information successfully saved.";
				else
					$resp['msg']= " Refugee Information successfully updated.";
			}else{
				$resp['status'] = 'failed';
				$resp['msg'] = "An error occured.";
				$resp['err'] = $this->conn->error."[{$sql}]";
	...
	function delete_refugee(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `refugees` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Refugee has been deleted successfully.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_cause(){
		...
		file_put_contents(base_app.'/cause.json',json_encode($data));
			...
			$contents .= '<meta property="og:type" content="article" />';
			$contents .= "\n";
			$contents .= '<meta property="og:title" content="sys" />';
			$contents .= "\n";
			$contents .= '<meta property="og:description" content="'.(addslashes($meta_description)).'" />';
			$contents .= "\n";
	...
	function update_refugee_status(){
		extract($_POST);
		
		$update = $this->conn->query("UPDATE `refugees` set status = '{$status}' where id = '{$id}'");
		if($update){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Status has been updated successfully.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
...
switch ($action) {
	case 'save_camp':
		echo $Master->save_camp();
	break;
	case 'delete_camp':
		echo $Master->delete_camp();
	break;
	case 'save_topic':
	...
	break;
	case 'save_refugees':
		echo $Master->save_refugee();
	break;
	case 'delete_refugee':
		echo $Master->delete_refugee();
	break;
	case 'update_refugee_status':
		echo $Master->update_refugee_status();
	break;
	default:
		// echo $sysset->index();
		break;
