                ...
                <div class="container-fluid">
                    <table class="table table-bordered table-hover table-striped">
                        ...
                        <tbody>
                            <?php 
						$i = 1;
						$qry = $conn->query("SELECT *,concat(fullname) as fullname from `donations`  ");
					...
					?>
                            <tr>
                                ...
                                <td class="">
                                    <p class="m-0 truncate-1"><?php echo $row['provider'] ?></p>
                                </td>
                                <td class="">
                                    <p class="m-0 truncate-1"><?php echo $row['phone_number'] ?></p>
                                </td>
                                <td class="">
                                    <p class="m-0 truncate-1"><?php echo $row['amount'] ?></p>
                                </td>

                                <td>
                                    <span id="status-<?php echo $row['id']; ?>"
                                        class="badge badge-<?php echo $row['status'] == 'Paid' ? 'success' : 'warning'; ?>">
                                        <?php echo $row['status']; ?>
                                    </span>
                                </td>
                                <!--td>
        <button class="btn btn-sm btn-success" onclick="setToPaid(<-?php echo $row['id']; ?>)"> Paid</button>
        <button class="btn btn-sm btn-danger" onclick="setToUnPaid(<-?php echo $row['id']; ?>)"> UnPaid</button>
    </td-->
    ...
    <noscript id="print-header">
        ...
        $(function() {
            $('#update_status').click(function() {
                uni_modal("Update Status of <b><?= isset($id) ? $id : "" ?></b>",
                    "report/donation.php?id=<?= isset($id) ? $id : "" ?>")
            })
            $('#add_shelter').click(function() {
                uni_modal("Add History Record for <b><?= isset($id) ? $fullname : "" ?></b>",
                    "report/donation.php?id=<?= isset($id) ? $id : "" ?>", 'mid-large')
            })
            $('.edit_shelter').click(function() {
                uni_modal("Edit History Record of <b><?= isset($id) ? $fullname : "" ?></b>",
                    "report/donation.php?id=<?= isset($id) ? $id : "" ?>&id=" + $(this).attr(
                        'data-id'), 'mid-large')
            })
            $('.delete_shelter').click(function() {
                _conf("Are you sure to delete this  Record?", "delete_shelter", [$(this).attr(
                    'data-id')])
            })
            $('#delete_volunteer').click(function() {
                _conf("Are you sure to delete this  Information?", "report/donation.php", [
                    '<?= isset($id) ? $id : '' ?>'
                ])
            })
            $('.view_data').click(function() {
                uni_modal("Report Details", "report/donation.php?id=" + $(this).attr('data-id'),
                    "mid-large")
            });
        ...
        function setToPaid(itemId) {
            ...
            $.post('http://localhost/GMS/status.php', {
                id: itemId
            }, function(response) {
                ...
                if (res.success) {
                    ...
                    $('button[onclick="setToPaid(' + itemId + ')"]').prop('disabled', true).text('Paid')
                        .removeClass('bg-green-500 hover:bg-green-700').addClass(
        ...
        function setToUnPaid(itemId) {
            ...
            $.post('http://localhost/GMS/status-.php', {
                ...
                if (res.success) {
                    ...
                    $('#status-' + itemId).text('Paid').removeClass('badge-warning').addClass('badge-success');
                    // Disable the button after successful update
                    $('button[onclick="setToUnPaid(' + itemId + ')"]').prop('disabled', true).text('UnPaid')
                        .removeClass('bg-red-500 hover:bg-red-700').addClass('bg-gray-400 cursor-not-allowed');
                } else {
                    alert('Failed to update status: ' + res.message);
                }
            }).fail(function(jqXHR, textStatus, errorThrown) {
