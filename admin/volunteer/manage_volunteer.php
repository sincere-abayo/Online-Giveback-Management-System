...
<div class="content py-3">
    <div class="card card-outline card-primary shadow rounded-0">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input type="text" name="middlename" id="middlename"
                                    ...
                                    class="form-control form-control-sm rounded-0" placeholder='optional'>
                            </div>
                            <div class="form-group col-md-4">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <textarea rows="" name="motivation" id="motivation"
                                    class="form-control form-control-sm rounded-0"
                                    required><?= isset($motivation) ? $motivation : "" ?></textarea>
                            </div>
                        </div>
                        <div class="container-fluid">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select id="status" name="status"
                                        ...
                                        <option value="1" <?= isset($status) && $status == 1 ? 'selected' : '' ?>>
                                            Accepted</option>
                                        <option value="2" <?= isset($status) && $status == 2 ? 'selected' : '' ?>>Denied
                                        </option>
                                    </select>
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="comment" class="control-label">feedback comment</label>
                                    <textarea rows="" name="comment" id="comment"
                                        class="form-control form-control-sm rounded-0"><?= isset($comment) ? $comment : "" ?></textarea>
                                </div>
                    </fieldset>
                </form>
            </div>
        ...
        <div class="card-footer text-right">
            <button class="btn btn-flat btn-primary btn-sm" type="submit" form="volunteer_form">Save Volunteer
                Details</button>
...
<script>
    $(function () {
        $('#volunteer_form').submit(function (e) {
            ...
            var isNewVolunteer = !formData.get('id'); // Check if this is a new volunteer
            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp.status == 'success') {
                        // Send email notification for new volunteers or status changes
                        if (isNewVolunteer || (resp.sid && status != 0)) {
                            var volunteerData = {
                            ...
                            $.ajax({
                                url: _base_url_ + "classes/Master.php?f=send_volunteer_email",
                                data: {
                                    volunteer_data: JSON.stringify(volunteerData),
                                    is_new: isNewVolunteer,
                                    status: status
                                },
                                method: 'POST',
                                dataType: 'json',
                            ...
                            alert_toast("Volunteer saved successfully!", 'success');
                            location.href = "./?page=volunteer/view_volunteer&id=" + resp.sid;
                        }
                    } else if (!!resp.msg) {
