...
Class Master extends DBConnection {
	...
	function save_cause(){
		...
		file_put_contents(base_app.'/cause.json',json_encode($data));
			...
			$contents .= '<meta property="og:type" content="article" />';
			$contents .= "\n";
			$contents .= '<meta property="og:title" content="Team" />';
			$contents .= "\n";
			$contents .= '<meta property="og:description" content="'.(addslashes($meta_description)).'" />';
			$contents .= "\n";
	...
	function save_program(){
		...
		$check = $this->conn->query("SELECT * FROM `program_list` where `name` = '{$name}' ".(is_numeric($id) && $id > 0 ? " and id != '{$id}'" : "")." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function save_activity(){
		...
		$check = $this->conn->query("SELECT * FROM `activity_list` where `name` = '{$name}' and `program_id` = '{$program_id}' ".(is_numeric($id) && $id > 0 ? " and id != '{$id}'" : "")." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function save_volunteer(){
		...
		$check = $this->conn->query("SELECT * FROM `volunteer_list` where id = '{$id}' ".(!empty($id) ? " and id != '{$id}' " : "")." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "An error occured.";
		}else{
