...
<!-- Navbar -->
      <nav class="main-header navbar navbar-expand navbar-dark border border-light border-top-0  border-left-0 border-right-0 navbar-light text-sm">
        ...
        <ul class="navbar-nav ml-auto">
          ...
          <div class="">
        <li class="nav-link">
          <a href="<?php echo base_url ?>admin/?page=refugee/request" class="btn btn-primary btn-sm position-relative">
  Requests

  <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
  <?php 
                    $request = $conn->query("SELECT id FROM `refugee_list` where status = 'O' ")->num_rows;
