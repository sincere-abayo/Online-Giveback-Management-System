            ...
            <div class="container-fluid">
                <table class="table table-bordered table-hover table-striped">
                    ...
                    <tbody>
                        <?php 
						$i = 1;
						$qry = $conn->query("SELECT c.*,d.name as program from `activity_list` c inner join program_list d on c.program_id = d.id where c.delete_flag = 0 order by c.`name` asc ");
					...
					?>
                        <tr>
                            ...
                            <td class="">
                                <p class="m-0 truncate-1"><?php echo $row['program'] ?></p>
                            </td>
                            <td class="">
...
$(document).ready(function() {
    $('#create_new').click(function() {
        uni_modal("Add New Activity", "activity/manage_activity.php")
    })
    $('.view_data').click(function() {
        uni_modal("Activity Details", "activity/view_activity.php?id=" + $(this).attr('data-id'))
    })
    $('.edit_data').click(function() {
        uni_modal("Update Activity Details", "activity/manage_activity.php?id=" + $(this).attr(
            'data-id'))
    })
    $('.delete_data').click(function() {
        _conf("Are you sure to delete this Camp permanently?", "delete_activity", [$(this).attr(
            'data-id')])
    })
